...
- **Known gaps found in review** (not yet fixed): see `FIXES_PENDIENTES.md`.
...
- `mission.md`'s other team extra ("la memoria del agente", a `/collect-memory` skill that folds session learnings back into `CLAUDE.md`) has **not** been built yet — there's no `.claude/skills/collect-memory`. Until it exists, updates to this file happen manually.
